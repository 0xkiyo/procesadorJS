...
public class AnalizadorLexico {
	...
	public void leerFichero(File fichero) {
		try {
			...
			System.err.println("Ha ocurrido un error leyendo el fichero");//Falta escribir en el Buffer errorWriter
		}
	}
	/*
	...
	public Token procS(char[] contenido, TablaSimbolos tS) throws ComentarioException, CadenaException, OpLogicoException, OtroSimboloException, FueraDeRangoException, IdException, DeclaracionIncompatibleException, IOException {
		Token toReturn = null;
		if (indice == contenido.length) {
			...
			if (digit < Math.pow(2, 15)) {
				...
				System.out.println("Error en linea: " + linea.toString() + " El numero " + Long.toString(digit) + " sobrepasa el maximo representable");//Falta escribir en el Buffer errorWriter
			}
		} else if (contenido[indice] == '\"') {
			//Cadena: genera token
			cadena = "";
			...
			if (cadena.equals("true")){
				...
				if (p[0] == null) {
					...
					System.out.println("Error en linea "+AnalizadorLexico.linea+". La variable o funcion '"+cadena+"' ha sido declarada previamente.");//Falta escribir en el Buffer errorWriter
				}
			...
			System.out.println("Error en linea: " + linea.toString() + " Se ha encontrado un simbolo que no pertenece a la gramatica: " + contenido[indice]);//Falta escribir en el Buffer errorWriter
		}
		return toReturn;
	}
	...
	public void procA(char[] contenido) throws ComentarioException {
		if (contenido[indice] == '/') {
			...
			System.out.println("Error en linea: " + linea.toString() + " Se esperaba detectar el simbolo '/'");//Falta escribir en el Buffer errorWriter
		}
	}
	...
	public void procE(char[] contenido) throws CadenaException {
		if (indice < contenido.length && contenido[indice] != '\"' && contenido[indice] != '\n' && contenido[indice] != '\r') {
            ...
            System.out.println("Error en linea: " + linea.toString() + " Se ha encontrado un salto de linea mientras se analizaba una cadena" + "\n");//Falta escribir en el Buffer errorWriter
        }
	}
	...
	public void procI(char[] contenido) throws OpLogicoException {
		if (contenido[indice] == '|') {
			...
			System.out.println("Error en linea: " + linea.toString() + " Se esperaba detectar otro simbolo '|'");//Falta escribir en el Buffer errorWriter
		}
	}
